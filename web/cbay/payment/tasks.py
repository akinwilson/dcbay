...
def check_intent_update_order_email_user(order_key):
    ...
    logger.info(
        f"Were there new transactions found from the last scan?-{ 'yes' if new_tx else 'no' }"
    )
    ...
    received_amount = Decimal(
        int(satoshis) * 10e-9
    )  # from satoshis to tbtc type Decimal

    logger.info(f"Converted to decimal type and units of BTC: {received_amount}")
    logger.info(
        f"Amount found at address {wallet.key(kid).address}: {received_amount}BTC"
    )
    ...
    if received_amount >= intent.paid_amount:
        ...
        logger.info(
            f"Intent: {intent} has had paid amount already recorded which i greater than the re-recorded amount at key"
        )
        logger.info(f"Intents paid amount: {intent.paid_amount}")
        logger.info(f"Receied from key scan: {received_amount}")
        logger.info(
            f"Total amount POSSIBLY received the sum: {received_amount + intent.paid_amount } but already swept the keys"
        )
        logger.info(
            f"It may be helpful to scan the address: {wallet.key(kid).address} for total amount paid and the amount supposed to be paid"
        )
        logger.info("*****Implications******")
        logger.info(
            f"Someone has attempt to make an additional payment to the address: {wallet.key(kid).address}"
        )
        logger.info(
            "But the key has already been swept, and this will appear as an under payment"
        )
        logger.info(f"Order key: {order_key}")
        logger.info("OPTIONS:")
        logger.info(
            "1) Add the newly recorded received amount to the paid_amount, and save the intent"
        )
        logger.info("----> intent.paid_amount = intent.paid_amount+received_amount")
        logger.info(
            "2) Manually update the corresponding order.billing_status=True intent.paid=True and send the user an email confirming their payment"
        )
        logger.info(
            "----> This would normally mean you found these logs because someone has contacted you because they never received their shipping"
        )
        logger.info(
            "----> the shipping address email only goes out if the below conditions of has_paid = True and intent.paid = False "
        )
        logger.info("CURRENTLY:")
        logger.info("=======we are in option 2===========")
        logger.info(
            "do not overwrite intent.paid_amount UNLESS the received amount is greater than the currrent, pre-set 0.0 value, but possibly previously recorded payment of none-zero"
        )
    ...
    logger.info(
        f"Allowed precentage differnce :{float(settings.PAYMENT_PRECENTAGE_LEEWAY) *100:.2f}%"
    )
    ...
    if precentage_diff <= Decimal(0.0):
    ...
    elif (precentage_diff > Decimal(0.0)) and (
        abs(precentage_diff) <= Decimal(settings.PAYMENT_PRECENTAGE_LEEWAY)
    ):
        # under payment within leeway
        logger.info(
            f"under payment within leeway of < {Decimal(settings.PAYMENT_PRECENTAGE_LEEWAY)}"
        )
        has_paid = True
        ...
        logger.info(
            f"under payment outside leeway > {Decimal(settings.PAYMENT_PRECENTAGE_LEEWAY)} "
        )
        has_paid = False
        logger.info("Notify underpayment ... ")

    logger.info(
        f"Has the intent been furfilled by customer? { 'YES' if has_paid else 'NO'}"
    )
    ...
    if has_paid and not intent.paid == True:
        ...
        subject = "Cbay: Payment received!"
        logger.info(
            f"Sending payment confirmation email for order order_id: { order.id}"
        )
        send_mail(
            subject,
            message,
...
def daily_shipments():
    ...
    logger.info(
        f"Number of orders with completed payment but not shipped: {len(orders)}"
    )
    if len(orders) == 0:
        ...
        if len(orders) > 0:
            logger.info(
                "Updating the shipped check_intent_update_order_email_user of orders"
            )
        for order in orders:
            order.shipped = True
            order.save()
